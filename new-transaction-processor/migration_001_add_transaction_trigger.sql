...
-- a new transaction is inserted with a payload starting with '6b3a' (in hex format)
...
RETURNS TRIGGER AS $$
BEGIN
    -- Check if the payload (bytea) starts with '6b3a' when converted to hex
    -- encode(NEW.payload, 'hex') converts bytea to hex string
    IF substr(encode(NEW.payload, 'hex'), 1, 4) = '6b3a' THEN
