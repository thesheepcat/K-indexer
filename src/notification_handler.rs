    ...
    pub async fn handle_notification(&self, notification: Notification) -> Result<()> {
        match notification {
            Notification::BlockAdded(block_notification) => {
                self.extract_k_protocol_transactions(block_notification).await
            }
            _ => Ok(()),
        }
    }

    async fn extract_k_protocol_transactions(&self, block_notification: BlockAddedNotification) -> Result<()> {
        for transaction in &block_notification.block.transactions {
            if transaction.payload.len() >= 4 {
                    ...
                    && transaction.payload[3] == K_PROTOCOL_PREFIX[3]
                {
                    ...
                    if let Err(err) = self.inner.k_transaction_channel.sender.send(transaction.clone()).await {
