...
async fn main() -> Result<()> {
    ...
    // Initialize database - create one PoloDbManager and use it for both purposes
    let db_interface = PoloDbManager::new(&args.database_path)
        .expect("Failed to initialize database interface");
    
    // Get collections for Kaspa connection (still using the same database instance)
    let k_posts_collection = db_interface.get_k_posts_collection();
    let k_replies_collection = db_interface.get_k_replies_collection();
    let k_broadcasts_collection = db_interface.get_k_broadcasts_collection();
    let k_votes_collection = db_interface.get_k_votes_collection();

    // Build Kaspa connection URL
    let rusty_kaspa_address_prefix = "ws://";
    let complete_rusty_kaspa_address =
        format!("{}{}", rusty_kaspa_address_prefix, args.rusty_kaspa_address);
    ...
    let db_interface_arc = Arc::new(db_interface);
    let web_server = WebServer::new(db_interface_arc);
