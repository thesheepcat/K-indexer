...
impl DatabaseInterface for PostgresDbManager {
    ...
    async fn get_all_posts(
        ...
        let query = format!(
            ...
            WITH all_posts AS (
                ...
                FROM k_contents c
                WHERE c.content_type = 'post'{cursor_conditions}
                {order_clause}
                LIMIT ${limit_param}
            ), post_stats AS (
                SELECT lp.id, lp.transaction_id, lp.block_time, lp.sender_pubkey,
    ...
    async fn get_contents_mentioning_user(
        ...
        let query = format!(
            ...
            WITH mentioned_content AS (
                -- Get content (posts and replies) that mention the specific user
            ...
            content_stats AS (
                ...
                LEFT JOIN (
                    ...
                    FROM k_contents r
                    WHERE r.content_type = 'reply'
                      AND EXISTS (SELECT 1 FROM mentioned_content mc WHERE mc.content_type = 'post' AND mc.transaction_id = r.referenced_content_id)
                    GROUP BY referenced_content_id
                ) r ON mc.content_type = 'post' AND mc.transaction_id = r.referenced_content_id
    ...
    async fn get_content_by_id(
        ...
        let query = r#"
            SELECT
                ...
                CASE WHEN c.content_type = 'post' THEN COALESCE(reply_counts.replies_count, 0) ELSE 0 END as replies_count,
                COALESCE(vote_counts.up_votes_count, 0) as up_votes_count,
                COALESCE(vote_counts.down_votes_count, 0) as down_votes_count,
                COALESCE(user_vote.is_upvoted, false) as is_upvoted,
                COALESCE(user_vote.is_downvoted, false) as is_downvoted,
            ...
            LEFT JOIN (
                ...
                GROUP BY referenced_content_id
            ) reply_counts ON c.content_type = 'post' AND c.transaction_id = reply_counts.referenced_content_id
            LEFT JOIN (
                SELECT
                    post_id,
                    COUNT(*) FILTER (WHERE vote = 'upvote') as up_votes_count,
        ...
        let query = format!(
            ...
            WITH all_posts AS (
                ...
                SELECT c.id, c.transaction_id, c.block_time, c.sender_pubkey,
                       c.sender_signature, c.base64_encoded_message
                FROM k_contents c
                WHERE c.content_type = 'post' AND c.sender_pubkey = $1{cursor_conditions}
                {order_clause}
                LIMIT ${limit_param}
            ),
            post_stats AS (
    ...
    async fn get_notifications(
        ...
        // Optimized query: filter k_mentions first, then join with content details from k_contents
        let query = format!(
            r#"
            WITH filtered_mentions AS (
                -- Step 1: Get recent mentions from non-blocked users (leverages comprehensive index)
            ...
            notifications_with_content AS (
                ...
                SELECT
                    ...
                    CASE WHEN fm.content_type = 'vote' THEN v.block_time ELSE NULL END as vote_block_time,
                    CASE WHEN fm.content_type = 'vote' THEN encode(v.post_id, 'hex') ELSE NULL END as content_id,
                    CASE WHEN fm.content_type = 'vote' THEN COALESCE(vc.base64_encoded_message, '') ELSE NULL END as voted_content
                FROM filtered_mentions fm
                LEFT JOIN k_contents c ON fm.content_type IN ('post', 'reply') AND fm.content_id = c.transaction_id AND c.content_type = fm.content_type
                LEFT JOIN k_votes v ON fm.content_type = 'vote' AND fm.content_id = v.transaction_id
                -- Get user profile for sender
                LEFT JOIN LATERAL (
                    SELECT base64_encoded_nickname, base64_encoded_profile_image
                ...
                -- For votes, get the content being voted on from k_contents
                LEFT JOIN k_contents vc ON fm.content_type = 'vote' AND v.post_id = vc.transaction_id
                {final_order_clause}
            )
            SELECT * FROM notifications_with_content
            "#,
