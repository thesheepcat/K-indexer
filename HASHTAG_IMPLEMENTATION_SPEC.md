...
**Regex Explanation:**
```
(?:^|\s)              Non-capturing group: start of string OR whitespace
#                     Literal hashtag character
([\p{L}\p{N}_]{1,30}) Capturing group: 1-30 chars of Unicode letters (\p{L}), numbers (\p{N}), or underscore (_)
(?:\s|$|[.,;!?])      Non-capturing group: whitespace, end, or punctuation
```
...
pub fn extract_hashtags_from_base64(base64_message: &str) -> Vec<String> {
    ...
    let valid_pattern = Regex::new(r"(?:^|\s)#([\p{L}\p{N}_]{1,30})(?:\s|$|[.,;!?])").unwrap();
    let mut valid_hashtags = HashSet::new();

    for capture in valid_pattern.captures_iter(&decoded_text) {
        if let Some(hashtag) = capture.get(1) {
            // Use Unicode-aware case folding
            valid_hashtags.insert(hashtag.as_str().to_lowercase());
        }
    }
...
Add comprehensive unit tests in the same file (include tests for warning behavior):
...
mod tests {
    ...
    fn test_extract_single_hashtag() {
        let message = base64::encode("Hello #world");
    ...
    fn test_extract_multiple_hashtags() {
        let message = base64::encode("Check #rust and #programming");
        let hashtags = extract_hashtags_from_base64(&message);
        assert_eq!(hashtags, vec!["rust", "programming"]);
    }

    #[test]
    fn test_hashtag_case_insensitive() {
        let message = base64::encode("#Rust #RUST #rust");
        let hashtags = extract_hashtags_from_base64(&message);
    ...
    fn test_hashtag_at_start() {
        let message = base64::encode("#start of message");
        let hashtags = extract_hashtags_from_base64(&message);
    ...
    fn test_hashtag_at_end() {
        let message = base64::encode("end of message #end");
        let hashtags = extract_hashtags_from_base64(&message);
    ...
    fn test_hashtag_with_punctuation() {
        let message = base64::encode("Hello #world! How are you?");
        let hashtags = extract_hashtags_from_base64(&message);
        assert_eq!(hashtags, vec!["world"]);
    }
    ...
    fn test_reject_no_space_before() {
        let message = base64::encode("word#tag");
        let hashtags = extract_hashtags_from_base64(&message);
    ...
    fn test_reject_url_with_hash() {
        let message = base64::encode("Visit google.com#section");
        let hashtags = extract_hashtags_from_base64(&message);
        assert!(hashtags.is_empty());
    }
    ...
    fn test_reject_too_long() {
        let message = base64::encode("#thistagiswaytoolongandshouldbrejected123456");
        let hashtags = extract_hashtags_from_base64(&message);
        assert!(hashtags.is_empty());
    }
    ...
    fn test_accept_max_length() {
        let message = base64::encode("#a123456789012345678901234567890"); // 30 chars
        let hashtags = extract_hashtags_from_base64(&message);
        assert_eq!(hashtags, vec!["a123456789012345678901234567890"]);
    }

    #[test]
    fn test_numeric_hashtags() {
        let message = base64::encode("#2024 and #123");
        let hashtags = extract_hashtags_from_base64(&message);
        assert_eq!(hashtags, vec!["2024", "123"]);
    }
    ...
    #[test]
    fn test_no_hashtags() {
        let message = base64::encode("This message has no hashtags");
        let hashtags = extract_hashtags_from_base64(&message);
        assert!(hashtags.is_empty());
    }
    ...
    fn test_just_hash_symbol() {
        let message = base64::encode("Just a # symbol");
        let hashtags = extract_hashtags_from_base64(&message);
        assert!(hashtags.is_empty());
    }
    ...
    fn test_mixed_valid_and_invalid_patterns() {
        ...
        let message = base64::encode("#rust and word#tag plus #programming and #verylongtagthatshouldberejectedbecauseitstoolong");
        let hashtags = extract_hashtags_from_base64(&message);
        ...
        assert!(!hashtags.contains(&"verylongtagthatshouldberejectedbecauseitstoolong".to_string()));
    }
